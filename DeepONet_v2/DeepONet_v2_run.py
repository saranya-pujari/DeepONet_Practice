...
# Scaling 
mean = {
    'forcing': tf.convert_to_tensor(np.mean(X_train[:, 2:-1], axis=0), dtype=tf.float32),
    'coords': tf.convert_to_tensor(np.mean(X_train[:, :2], axis=0), dtype=tf.float32)
}

var = {
    'forcing': tf.convert_to_tensor(np.var(X_train[:, 2:-1], axis=0), dtype=tf.float32),
    'coords': tf.convert_to_tensor(np.var(X_train[:, :2], axis=0), dtype=tf.float32)
}
...
class LossTracking:
    ...
    def update(self, total_loss, IC_loss, PDE_loss):
    ...
    def print(self):
        print(f"IC={self.mean_IC_loss.result().numpy():.4e}, PDE={self.mean_PDE_loss.result().numpy():.4e}, total_loss={self.mean_total_loss.result().numpy():.4e}")
...
for epoch in range(1, n_epochs + 1):  
    ...
    for X_init, X in zip(ini_ds, train_ds):
        ...
        # Calculate gradients
        PDE_loss, IC_loss, total_loss, gradients = model.train_step(X, X_init, 
        ...
        # Loss tracking
        loss_tracker.update(total_loss, IC_loss, PDE_loss)
    ...
    val_res = model.PDE_residual_calculator(X_val_tensor[:, :1], X_val_tensor[:, 1:-1], X_val_tensor[:, -1:], PI_DeepONet)
    ...
    print(f"val_IC: {val_IC.numpy():.4e}, val_PDE: {val_PDE.numpy():.4e}, lr: {PI_DeepONet.optimizer.learning_rate.numpy():.2e}")
